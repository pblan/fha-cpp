...
int main() {
  ...
  std::cout << "str_x: " << str_x << ", str_y: " << str_y
            << ", str_z: " << str_z << std::endl;
...
// Schreiben Sie eine Funktion 'cut', die bei zwei per Referenz übergebenen
// 'double'-Zahlen jeweils die Nachkommastellen auf 0 setzt. Nutzen Sie die
// Funktion 'floor' aus 'cmath'.
void cut(double &num_a, double &num_b) {
  num_a = floor(num_a);
  num_b = floor(num_b);
}

// Schreiben Sie eine Funktion 'shift', die drei per Referenz übergebene Strings
// 'der Reihe nach' tauscht [...].
void shift(std::string &str_x, std::string &str_y, std::string &str_z) {
