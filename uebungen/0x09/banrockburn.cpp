...
public:
    ...
    {
        std::cout << "[ctor linked_list] empty" << std::endl;
    };

    linked_list(const node<T> &root)
        : root{root}
    {
    ...
    linked_list &operator+=(std::shared_ptr<node<T>> n)
    {
        if (!root)
        {
            root = n;
...
template <typename T>
std::ostream &operator<<(std::ostream &os, const linked_list<T> &l)
{
    std::shared_ptr<node<T>> it = l.root;
...
int main()
{
    ...
    l += std::make_shared<node<int>>(1);
    l += std::make_shared<node<int>>(2);
    l += std::make_shared<node<int>>(3);

    std::cout << "l: " << l << std::endl;
    std::cout << "l.root: " << l.root << ", l.root->value: " << l.root->value << std::endl;
    std::cout << "l.tail: " << l.tail << ", l.tail->value: " << l.tail->value << std::endl;

    std::weak_ptr<node<int>> it = l.tail;
    std::shared_ptr<node<int>> it_p;
